import { at as reactive, h as computed, j as nextTick, q as watch, rt as markRaw, st as ref } from "./modules/shiki-V2eyNvW-.js";
import { E as createSharedComposable, h as useLocalStorage } from "./modules/vue-j1EMyUNA.js";
import { Q as configs_default, g as isInputting, t as useNav } from "./useNav-BevVU56q.js";
import { c as onPatchDrawingState, l as patchDrawingState, s as drawingState } from "./index-BrwRSUAk.js";
...
var useDrawings = createSharedComposable(() => {
	...
	const brush = useLocalStorage("slidev-drawing-brush", {
		color: brushColors[0],
		size: 4,
		mode: "stylus"
	});
	...
	const syncUp = computed(() => configs_default.drawings.syncAll || isPresenter.value);
	...
	const drauuOptions = reactive({
		brush,
		acceptsInputTypes: computed(() => drawingEnabled.value && (!configs_default.drawings.presenterOnly || isPresenter.value) ? void 0 : ["pen"]),
		coordinateTransform: false
	});
	const drauu = markRaw(createDrauu(drauuOptions));
	function clearDrauu() {
		drauu.clear();
		if (syncUp.value) patchDrawingState(currentSlideNo.value, "");
	}
	...
	drauu.on("changed", () => {
		...
		if (!disableDump) {
			...
			if ((drawingState[key] || "") !== dump && syncUp.value) patchDrawingState(key, drauu.dump());
		}
	});
	onPatchDrawingState((state) => {
		disableDump = true;
		if (state[currentSlideNo.value] != null) drauu.load(state[currentSlideNo.value] || "");
		disableDump = false;
		updateState();
	});
	nextTick(() => {
	...
	window.addEventListener("keydown", (e) => {
		if (!drawingEnabled.value || isInputting.value) return;
		const noModifier = !e.ctrlKey && !e.altKey && !e.shiftKey && !e.metaKey;
		let handled = true;
		if (e.code === "KeyZ" && (e.ctrlKey || e.metaKey)) if (e.shiftKey) drauu.redo();
		else drauu.undo();
		else if (e.code === "Escape") drawingEnabled.value = false;
		else if (e.code === "KeyL" && noModifier) drawingMode.value = "line";
		else if (e.code === "KeyA" && noModifier) drawingMode.value = "arrow";
		else if (e.code === "KeyS" && noModifier) drawingMode.value = "stylus";
		else if (e.code === "KeyR" && noModifier) drawingMode.value = "rectangle";
		else if (e.code === "KeyE" && noModifier) drawingMode.value = "ellipse";
		else if (e.code === "KeyC" && noModifier) clearDrauu();
		else if (e.code.startsWith("Digit") && noModifier && +e.code[5] <= brushColors.length) brush.value.color = brushColors[+e.code[5] - 1];
		else handled = false;
		if (handled) {
			e.preventDefault();
			e.stopPropagation();
